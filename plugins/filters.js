...
await message.client.sendMessage(message.jid, fs.readFileSync('./uploads/mention.mp3'), MessageType.sticker, { mimetype: Mimetype.webp, quoted : message.data, ptt: false})
    ...
    const array = ['Mm','sed','jackz','Good morning','wizard','nanjan kotta','dora','mention','paathu','welcome','ചോദിക്കും','poda myre','Stanly','eid mubarak','mess','umma','zatan','ambada','mathi','Welcome','Paathu','viper','Name','eh','Bc','panni','Happy','good night','Ok bei','Dance','oh','Hi','ask you','Poda','Hlo','ayyo','chirikk','eatha']
    array.map( async (a) => {
    let pattern = new RegExp(`\\b${a}\\b`, 'g');
    if(pattern.test(message.message)){
